...
public class GossipController  extends BaseController{
	...
	public String gossipBlockSearch(@RequestBody Map<String,String> inputParam) {	
		...
		try {
			Integer pageCur = null;
			if(inputParam.containsKey("curPage")) {
				pageCur = Integer.parseInt(inputParam.get("curPage"));
			}
			else {
				pageCur = 1;
			}
			List<String> gossipText = gossipService.gossipBlockSearch(searchBlock, pageCur);
			jsonMap = ImmutableMap.of("texts", gossipText);
		}catch(Exception ex){
			return errorRespond();
		}
		return passRespond(jsonMap);
	...
	public String gossipStatusSearch(@RequestBody Map<String,String> inputParam) {	
		...
		try {
			...
			List<String> gossipText = gossipService.gossipStatusSearch(searchStatus,curPage);
			jsonMap = ImmutableMap.of("texts", gossipText);
		}catch(Exception ex){
			return errorRespond();
		}
